...
Requests that include cookies aren't cached so stale cookies don't impact app behavior.

In some cases, cookies are used client-side only. For example, client-side JavaScript may produce and consume cookies but never interact with the server-side PHP app. Google Analytics is one specific example that uses cookies to store information about the page. In order to maximize default caching, Varnish drops these special case cookies from the HTTP request to the app. 

Currently only the following cookies are dropped:

* Empty cookies (i.e. cookies with no values).
* Cookies from Google Analytics.
* Cookies with the name <code>has_js</code>.
* URL's ending in media extensions have cookies stripped by default. These include:
    * otf
    * jpg
    * jpeg
    * gif
